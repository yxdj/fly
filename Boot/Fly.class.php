...
class Fly{
	...
	static public function loadExt($load,$check=true){
		if(is_string($load)){
			$ext=$load;
			$func='load_'.$ext;
			$file2=ucfirst($ext).'/'.$ext.'.fun.php';

			//load函数是否存在
			if(function_exists($func) && $check){
				throw new FlyException($ext.' is be load!');
			}else if(function_exists($func)){
				return;
			}
			
			//文件是否存在
			if(is_file(FLY_LIB.$file2)){
				$file=FLY_LIB.$file2;
				require($file);
			}else if(is_file(APP_LIB.$file2)){
				$file=APP_LIB.$file2;
				require($file);
			}else{
				throw new FlyException('load '.$ext.' error. file<'.$file2.'>: is not exists!');
			}
			...
			throw new FlyException('load config ng!');
		}
		return;
	}
	...
	static public function start($argv,$argc,$args=array()){
		...
		Fly::loadExt(C('flyLoadMap'),false);
