...
class FlyImage {
	...
	private $background=array(0,0,0);	//设置透明色，默认为黑色
    public  $status=true;       //是否成功
    public  $error='';          //错误信息
	...
	public function sets($confs=array()){
	...
	//设定来源图片，及点位
	public function set($source,$coor=null){
		//取得原图片地址->磁盘地址
        $this->source =$source;
		
		//取得图片的宽，高，类型
        @list($this->swidth, $this->sheight, $this->type) = getimagesize($this->source);

		
		//根据图片类型获得相应资源名柄
		$this->img=$this->getImg($this->source, $this->type);	
	}
    ...
    //要据图片地址和类型，返回图片的资源句柄
    private function getImg($source, $type) {
        switch ($type) {
            case 1 :
                //$this->type='gif';
            ...
            default:
                $this->status=false;
                $this->error='图片处理类不能获取此图片: '.$source;
                return false;
        }
        return $img;
    }
