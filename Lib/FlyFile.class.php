...
class FlyFile{
    //查看文件目录修改时间
    public static function changeTime($path,$num=false){
    ...
    //求文件大小
    public static function size($path,$max=104857600){//$max=100MB
            if(!file_exists($path)){//不是文件（不是路径也不是文件）
                return 'ERROR:'.$path.'不是文件或文件夹!';
            }
            $result=0;
    ...
    //查看文件状态
    public static function status($file,$info=false){
		$result=-1;
    ...
    //创建文件目录
    public static function create($path,$root=''){
            //提取路径节点
